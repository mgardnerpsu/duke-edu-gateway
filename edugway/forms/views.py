...
class FieldViewSet(mixins.ListModelMixin, mixins.RetrieveModelMixin, mixins.UpdateModelMixin, 
	...
	def move_seq_up(self, request, pk=None):
		...
		form = field_up.form
		min_sequence = form.fields.all().aggregate(Min('sequence'))['sequence__min']
	...
	def choices(self, request, pk=None):
		if request.method == 'POST':
			...
			serializer = ChoiceSerializer(data=request.data, many=False, context={'request': request})
			serializer.is_valid(raise_exception=True)
...
class ChoiceViewSet(mixins.ListModelMixin, mixins.RetrieveModelMixin, mixins.UpdateModelMixin, 
	...
	def move_seq_up(self, request, pk=None):
		...
		min_sequence = field.choices.all().aggregate(Min('sequence'))['sequence__min']
