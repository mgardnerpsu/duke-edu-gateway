...
class ChoiceTests(APITestCase):
	...
	def test_get_choice(self):
		url = reverse('field-choices-detail', args=[self.choice_id])
		response = self.client.get(url)
		self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class ReSequenceChoicesTests(APITestCase):
	...
	def test_move_choice_down(self):
		...
		url = reverse('field-choices-move-down', args=[self.choices[5]['id']])
		...
		# move choice 3 down
		url = reverse('field-choices-move-down', args=[self.choices[3]['id']])
		response = self.client.put(url)
		self.assertEqual(response.status_code, status.HTTP_200_OK)
		# verify 3 moved to 4
		...
		# verify 4 moved to 3
		url = reverse('field-choices-detail', args=[self.choices[4]['id']])
		response = self.client.get(url)
		self.assertEqual(response.status_code, status.HTTP_200_OK)
	...
	def test_move_choice_up(self):
		...
		url = reverse('field-choices-move-up', args=[self.choices[1]['id']])
		response = self.client.put(url)
		self.assertEqual(response.status_code, status.HTTP_200_OK)
		...
		# move choice 4 up
		url = reverse('field-choices-move-up', args=[self.choices[4]['id']])
		response = self.client.put(url)
		self.assertEqual(response.status_code, status.HTTP_200_OK)
		# verify 4 moved to 3
		self.assertEqual(response.data['sequence'], 3)
		...
		self.assertEqual(response.data['label'], 'sample choice - 4')
		# verify 3 moved to 4
		url = reverse('field-choices-detail', args=[self.choices[3]['id']])
		response = self.client.get(url)
		self.assertEqual(response.status_code, status.HTTP_200_OK)
		self.assertEqual(response.data['sequence'], 4)
		self.assertEqual(response.data['name'], 'choice-4')
	...
	def test_delete_choice(self):
		# delete choice 2
		url = reverse('field-choices-detail', args=[self.choices[2]['id']])
		response = self.client.delete(url)
