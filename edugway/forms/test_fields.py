...
class FieldTests(APITestCase):
	...
	def test_get_field(self):
		url = reverse('content:form-fields-detail', args=[self.field_id])
		response = self.client.get(url)
		self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class ReSequenceFieldsTests(APITestCase):
	...
	def test_move_field_down(self):
		...
		url = reverse('content:form-fields-move-seq-down', args=[self.fields[5]['id']])
		...
		# move field 3 down
		url = reverse('content:form-fields-move-seq-down', args=[self.fields[3]['id']])
		response = self.client.put(url)
		self.assertEqual(response.status_code, status.HTTP_200_OK)
		# verify 3 moved to 4
		...
		# verify 4 moved to 3
		url = reverse('content:form-fields-detail', args=[self.fields[4]['id']])
		response = self.client.get(url)
		self.assertEqual(response.status_code, status.HTTP_200_OK)
	...
	def test_move_field_up(self):
		...
		url = reverse('content:form-fields-move-seq-up', args=[self.fields[1]['id']])
		response = self.client.put(url)
		self.assertEqual(response.status_code, status.HTTP_200_OK)
		...
		# move field 4 up
		url = reverse('content:form-fields-move-seq-up', args=[self.fields[4]['id']])
		response = self.client.put(url)
		self.assertEqual(response.status_code, status.HTTP_200_OK)
		# verify 4 moved to 3
		self.assertEqual(response.data['sequence'], 3)
		...
		self.assertEqual(response.data['label'], 'sample field - 4')
		# verify 3 moved to 4
		url = reverse('content:form-fields-detail', args=[self.fields[3]['id']])
		response = self.client.get(url)
		self.assertEqual(response.status_code, status.HTTP_200_OK)
		self.assertEqual(response.data['sequence'], 4)
		self.assertEqual(response.data['name'], 'field-4')
	...
	def test_delete_field(self):
		# delete field 2
		url = reverse('content:form-fields-detail', args=[self.fields[2]['id']])
		response = self.client.delete(url)
